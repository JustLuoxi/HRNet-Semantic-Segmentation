...
class Lx(BaseDataset):
    def __init__(self,
        ...
        super(Lx, self).__init__(ignore_label, base_size,
    ...
    def __getitem__(self, index):
        ...
        return image.copy(), gt.copy(), np.array(size), name

    def gen_sample(self, image, gt,
        ...
        if center_crop_test:
            image, gt = self.image_resize(image,
                                             self.base_size,
                                             gt)
            image, gt = self.center_crop(image, gt)
    ...
    def image_resize(self, image, long_size, gt=None):
        h, w = image.shape[:2]
        if h > w:
            new_h = long_size
        ...
        image = cv2.resize(image, (new_w, new_h),
                           interpolation=cv2.INTER_LINEAR)
        if gt is not None:
            label = cv2.resize(gt, (new_w, new_h),
                               interpolation=cv2.INTER_LINEAR)
        else:
            return image
